...
extern "C" {
  ...
  SEXP spVarBayes_MFA_LR_update_paracpp(SEXP y_r, SEXP X_r,
    ...
    double *ei = (double *) R_alloc(n, sizeof(double)); zeros(ei, n);
    // double *Bei = (double *) R_alloc(n, sizeof(double)); zeros(Bei, n);
    ...
    for (int i = 0; i < n; i++) {
      ...
      // for (int j = 0; j < n; j++) {
      //   Bei[j] = Sigma_star[j + i * n];
      // }
      //
      // trace += Q(B, F, ei, Bei, n, nnIndx, nnIndxLU);
      trace += Q(B, F, ei, &Sigma_star[i * n], n, nnIndx, nnIndxLU);
      ei[i] = 0.0;
    }

    b_zeta_update = zetaSqIGb + (trace + zeta_Q)*theta[zetaSqIndx]*0.5;
